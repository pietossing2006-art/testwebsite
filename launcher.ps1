...
function Stop-AllServices {
  ...
  $pidFiles = @('server.pid', 'client.pid', 'tunnel.pid')
  ...
  $ports = @(3001, 5173, 5000, 4173, 9444)
...
function Start-AllServices {
  ...
  Write-Host "  [2/2] Starting Web Client ($cMode on Port $cPort)..." -ForegroundColor Yellow
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    if ($serverReady -and $clientReady) { break }
  }
...
while ($true) {
  ...
  switch ($choice.Trim()) {
    ...
    default {
      Write-Host "  ⚠️ Invalid choice, please choose 0-6." -ForegroundColor Red
      Start-Sleep -Seconds 1
    }
  }
}
