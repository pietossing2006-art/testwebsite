...
export default async function Layout({ children }: Readonly<{ children: ReactNode }>) {
  ...
  const accountSwitcherUsers = currentUser
    ? [
        {
          id: String(currentUser.id),
          name: currentUser.display_name || currentUser.username || currentUser.email,
  ...
  return (
    ...
    >
      <AppSidebar variant={variant} collapsible={collapsible} currentUser={navUser} />
      <SidebarInset
        className={cn(
        ...
        >
          <div className="flex w-full items-center justify-between px-4 lg:px-6">
            ...
            <div className="flex items-center gap-2">
              <LayoutControls />
              <ThemeSwitcher />
              <AccountSwitcher users={accountSwitcherUsers} />
            </div>
          </div>
        </header>
