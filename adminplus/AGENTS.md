...
Adminplus (Admin+) is the splitwise admin panel — Next.js 16, React 19, TypeScript, Tailwind CSS v4, shadcn/ui.
It started from the `arhamkhnz/next-shadcn-admin-dashboard` template; every demo page has been removed and the
modules under `src/app/(main)/dashboard/` are ports of the old AdminV3 panel (`../client/src/pages/AdminV3/`).

It talks to the splitwise API in `../server` (default `http://localhost:3001`, set via `SPLITWISE_API_BASE`):

- Auth goes through the server actions in `src/server/auth-actions.ts`; the session token lives in an httpOnly
  cookie and never reaches the browser. `src/proxy.ts` gates `/dashboard/*` behind it.
- Server components read data with `serverAdminGet` (`src/lib/adminplus/server-api.ts`).
- Client components call `/api/admin/*` and `/api/staff/*`, which the catch-all proxies in `src/app/api/`
  forward upstream with the caller's own Bearer token. The splitwise server still enforces every role check.
- It runs on port 3010 and is wired into the repo launchers (`start-vxpers.exe`, `launcher.ps1`, `start-dev.ps1`)
  via `run-adminplus.ps1`.
